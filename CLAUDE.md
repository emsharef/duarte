...
The project is being rebuilt toward feature parity with Collector Systems/Arternal plus AI capabilities (ported from the Room Service project in `../arternal`). Specs and plans live in `docs/superpowers/specs/` and `docs/superpowers/plans/`. Phase 1 (workspaces + schema rebuild) and Phase 2 (interaction model: rebuilt object detail with inline editing + counted tabs in components/object/, inventory list system with filter rail/column picker/saved views/batch tray in components/list/) are complete. Phase 3 is registrar module UIs (disposals, exhibitions, condition, provenance, consignments, shipments) plus a dedicated Lists page with reordering; Phases 4–5 are AI (data entry, embeddings/chat/MCP, enrichment, valuations).
